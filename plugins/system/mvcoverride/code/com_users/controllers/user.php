...
class UsersControllerUser extends UsersController
{
	...
	public function login()
	{
		...
		session_start();
        if (isset($_SESSION["download_licit"])&& !empty($_SESSION["download_licit"])){
		//	$arquivo = $_SESSION["download_licit"];
			//var_dump($page);die;
			$app->redirect($_SESSION["url_licit"]);
			//header('location: ' . $arquivo,false);
			//header('location: ' . $_SESSION["url_licit"]);
		}
		else{
	...
	public function remind()
	{
		...
		session_start();
        		if (isset($_SESSION["download_licit"])&& !empty($_SESSION["download_licit"])){
					$route	= 'index.php?option=com_users&view=login&Itemid=178';
				}
				else{
