...
class UsersControllerRegistration extends UsersController
{
	...
	public function register()
	{
        ...
        $rand        = rand(1, 99);
		$requestData['username'] = 'ext.'.$rand.explode('@', $requestData['email1'])[0]; 
		$requestData['email1']   = $requestData['email1'].'.frontend'; 
        $requestData['email2']   = $requestData['email2'].'.frontend';
		...
		if ($data === false)
		{
			...
			// Remove frontend atr
			$requestData['email1'] = str_replace(".frontend", "", $requestData['email1']);
			$requestData['email2'] = str_replace(".frontend", "", $requestData['email2']);
		...
		{
			$this->setMessage('Sua conta foi criada e um link de confirmação foi enviado para o endereço de email informado. Observe que você deve ativar a conta, clicando no link de confirmação do email.');
			$this->setRedirect(JRoute::_('index.php?option=com_users&view=registration&layout=complete', false));
		}
