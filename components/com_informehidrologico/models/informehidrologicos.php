...
class InformehidrologicoModelInformehidrologicos extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if (!Factory::getUser()->authorise('core.edit', 'com_informehidrologico'))
		{
		...
		foreach ($items as $item)
		{
			...
			if (isset($item->tipo) && $item->tipo != '')
			{
				...
				if(!empty($this->_item->tipo)){
					$query->where($db->quoteName('id') . ' IN (' . $db->quote($this->_item->tipo) . ')');
				}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(JText::_("COM_INFORMEHIDROLOGICO_SEARCH_FILTER_DATE_FORMAT"), "warning");
