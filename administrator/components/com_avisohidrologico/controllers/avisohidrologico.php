...
class AvisohidrologicoControllerAvisohidrologico extends JControllerForm
{
  ...
  public function save($data = array(), $key = 'id')
{
   ...
   $avisohidrologicoCrllr = new AvisohidrologicoControllerAvisohidrologico();

    //file upload
   $uploadedFiles = $avisohidrologicoCrllr::makeUpload($_FILES['jform']);
    ...
    elseif(strtotime($data['inicio']) > strtotime($data['validade'])):
      ...
      JFactory::getApplication()->redirect($url);
        ...
        if(empty($data['id']))
        {
          ...
          $avisohidrologicoCrllr::insertData($data);

          $url = JRoute::_('index.php?option=com_avisohidrologico&view=avisohidrologicos');
          ...
          $data['arquivo'] = $uploadedFiles;
          $avisohidrologicoCrllr::updateData($data);
          $url = JRoute::_('index.php?option=com_avisohidrologico&view=avisohidrologicos');
...
public function insertData($arr)
{
  ...
  $avisohidrologicoCrllr = new AvisohidrologicoControllerAvisohidrologico();
  $identificador_gerado  = $avisohidrologicoCrllr::generateIdentificador();
  ...
  $id_aviso = $avisohidrologicoCrllr::getAvisoLastId();
  ...
  $query->insert($db->quoteName('#__avisohidrologico'))
  ...
  foreach ($arr['arquivo'] as $arquivo_item) {
    ...
    $contador++;
    $avisohidrologicoCrllr->addAnexo($anexo);
  ...
  public function updateData($arr)
 {
    ...
    $query->update($db->quoteName('#__avisohidrologico'))->set($fields)->where($conditions);
    $db->setQuery($query);
    $db->execute();
    ...
    foreach ($arr['arquivo'] as $arquivo_item) {
      ...
      if(! empty($arr['arquivos_deletados'])) {
        ...
        foreach ($arquivos_deletados_id_array as $arquivo_deletado_id) {
          $avisohidrologicoCrllr->removeAnexo($arquivo_deletado_id);
        }

      }

      //atualiza os anexos 
      $anexosSalvos = $avisohidrologicoCrllr->getAnexosByIdArray($arr['id']);

      for ($i=0; $i < count($arr['titulo_old']); $i++) { 

        if($arr['titulos'][$i] != $arr['titulo_old'][$i]) {

          $avisohidrologicoCrllr->updateAnexoById($arr['anexo_id'][$i],$arr['titulos'][$i]);
  ...
  public function addAnexo($anexo)
  {
    ...
    $query->insert($db->quoteName('#__avisohidrologico_anexo'))
          ->columns($db->quoteName($columns))
          ->values(implode(',', $values));
  ...
  public function removeAnexo($anexo_id)
  {
    ...
    $query->delete($db->quoteName('#__avisohidrologico_anexo'));
  ...
  function updateAnexoById($id,$titulo)
  {
    ...
    $query->update($db->quoteName('#__avisohidrologico_anexo'))->set($fields)->where($conditions);
    $db->setQuery($query);
    $db->execute();
