...
class AvisohidrologicoControllerAvisohidrologicos extends JControllerAdmin
{		
	public function getModel($name = 'Avisohidrologico', $prefix = 'AvisohidrologicoModel', $config = array('ignore_request' => true))
	{
	...
	public function enviarEmail()
	{
		...
		$query->select('*')->from($db->quoteName('lista_emails', 'le'))->where("le.id_item = $id AND le.tipo_item = 'AVISO HIDROLOGICO'");
		$db->setQuery($query);
		...
		if(empty($resultado)){
			...
			if($return['tipo'] == "sucesso"){	
				$url = JRoute::_('index.php?option=com_avisohidrologico');
