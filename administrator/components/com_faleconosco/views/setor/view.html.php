...
class contatoViewsetor extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		$input->set('hidemainmenu', true);
 		...
 		JToolBarHelper::title('Fale Conosco - Setores', 'list-2');
	...
	public static function cahngeSetor($f, $data){
		if($f === 'add'){
			try {
				$db = JFactory::getDbo();
				$query = $db->getQuery(true);
				$query->insert($db->quoteName('#__contato_setor'));
				$query->columns(array('nome'));
			...
			try {
				...
				$query->delete($db->quoteName('#__contato_setor'));
	...
	protected function setDocument()
	{
		...
		$document->setTitle(JText::_('Fale Conosco - Setores'));
	}
