...
/**
 * Methods supporting a list of Previsaodotempo records.
 *
 * @since  1.6
 */
class PrevisaodotempoModelPrevisoes extends JModelList
{
...
/**
	...
	*/
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'datavlida', 'a.datavlida',
				'tipo', 'a.tipo',
				'horario', 'a.horario',
				'observaes', 'a.observaes',
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Load the parameters.
		$params = JComponentHelper::getParams('com_previsaodotempo');
	...
	protected function getListQuery()
	{
		...
		$query->from('#__previsaodotempo_previsao AS a');
		...
		// Filter by search in tipo
		$tipo = $this->getState('filter.tipo');

		if (!empty($tipo) && $tipo != '')
		{
			$query->where('a.tipo = ' . $db->quote($tipo) );
		}
		...
		if (!empty($autor))
		{
			$query->where('a.created_by = ' . (int) $autor);
		}
		...
		if($inicio == '' && $fim != ''){
			$query->where("a.datavlida < '".$fim."'");
		} else if( (strtotime($agora_date) < strtotime($fim_date)) || (strtotime($fim_date) < strtotime($ini_date)) ) {
			// data fora do range
			$fim = $agora_date->format('Y-m-d');
			$inicio = $inicio;
			$query->where("a.datavlida between  '". $ini_date->format('Y-m-d')  ."' and '" . $fim ."'");
		} else if($inicio != '' && $fim != '') {
			$fim    = $this->manipulaData($fim, 1);
			$inicio = $inicio;
			$query->where("a.datavlida between  '". $ini_date->format('Y-m-d') ."' and '" . $fim_date->format('Y-m-d') ."'");
		}
