 ...
 * @since  1.6
 */
class PrevisaodotempoControllerVariaveis extends JControllerAdmin
{
	/**
	 ...
	 * @since    1.6
	 */
	public function getModel($name = 'variavel', $prefix = 'PrevisaodotempoModel', $config = array())
	{
	...
	public function publish(){
		...
		$modelitem                              =   $this->getModel('Variavel');        
		$status                                 = 1;
		// set status
		switch($task){
				case 'publish'  :                   
		...
		foreach($ids as $id){
			...
			$query->update($db->quoteName('#__previsaodotempo_variavel'))->set($fields)->where($conditions);
